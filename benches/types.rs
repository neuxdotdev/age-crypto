...
fn bench_encrypted_data(c: &mut Criterion) {
    ...
    c.bench_function("encrypted_data_as_bytes", |b| b.iter(|| enc.as_bytes()));
    c.bench_function("encrypted_data_to_vec", |b| b.iter(|| enc.to_vec()));
...
fn bench_armored_data(c: &mut Criterion) {
    ...
    c.bench_function("armored_as_str", |b| b.iter(|| armored.as_str()));
...
fn bench_passphrase(c: &mut Criterion) {
    ...
    c.bench_function("passphrase_expose", |b| b.iter(|| p.expose()));
    c.bench_function("passphrase_clone", |b| b.iter(|| p.clone()));
